...
def parse_param_key(pk: str) -> dict:
    """Extract parameter values from a param_key string like 'v8_m4_s0.1_f2_l2_c20'."""
    params = {}
    for token in pk.split("_"):
        if token.startswith("v") and token[1:].replace(".", "").replace("-", "").isdigit():
            params["vol_to_half_spread"] = float(token[1:])
        elif token.startswith("m") and token[1:].replace(".", "").replace("-", "").isdigit():
            params["min_half_spread_bps"] = float(token[1:])
        elif token.startswith("s") and token[1:].replace(".", "").replace("-", "").isdigit():
            params["skew"] = float(token[1:])
        elif token.startswith("f") and token[1:].replace(".", "").replace("-", "").isdigit():
            params["spread_factor"] = float(token[1:])
        elif token.startswith("l") and token[1:].isdigit():
            params["num_levels"] = int(token[1:])
        elif token.startswith("c") and token[1:].replace(".", "").replace("-", "").isdigit():
            params["c1_ticks"] = float(token[1:])
    return params


def load_state_files(grid_dir: str) -> list:
    ...
    for fname in sorted(os.listdir(grid_dir)):
        ...
        # Extract param_key from filename: state_BTC-USD_<param_key>.json
        parts = fname[len("state_"):-len(".json")]
        # Remove symbol prefix (e.g. "BTC-USD_")
        idx = parts.find("_v")
        if idx >= 0:
            symbol = parts[:idx]
            pk = parts[idx + 1:]
...
def load_trade_counts(grid_dir: str) -> dict:
...
def print_summary(slots: list, trade_counts: dict, top_n: int, sort_key: str, maker_fee_rate: float):
    if not slots:
    ...
    for s in slots:
        initial_pv = s.get("initial_portfolio_value", 0) or s.get("initial_capital", 0)
        pv = s.get("portfolio_value", 0)
        realized = s.get("realized_pnl", 0)
        s["unrealized_pnl"] = pv - initial_pv - realized if initial_pv > 0 else 0
        s["total_pnl"] = realized + s["unrealized_pnl"]
...
def main():
    parser = argparse.ArgumentParser(description="Check StandX grid dry-run results")
    ...
    parser.add_argument("--fee", type=float, default=0.0001,
                        help="Maker fee rate as fraction (default: 0.0001 = 0.01%%)")
